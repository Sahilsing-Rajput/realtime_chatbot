...
get_hr_information_def = {
    "name": "get_hr_information",
    "description": "Get BAGIC HR policy information",
    "parameters": {
        "type": "object",
        "properties": {
            "question": {
                "type": "string",
                "description": "User query or question e.g. 'Leave policies'"
            }
        },
        "required": ["question"]
    }
...
async def get_hr_information_handler(question):
    # Set the URL as a variable
    url = "add your url"
    
    payload = {
        "headers": {
            "conversationalKey": "srn_faiss_index"
        },
        "question": question,
        "chatHistory": []
    }
    try:
        ...
        data = response.json()
        answer = json.loads(data["body"])["answer"]
        
        # Printing the result to the console (can be changed to logging or returning the result)
        print(f"HR Answer: {answer}")
        return answer
    except requests.RequestException as e:
        error_message = f"Failed to fetch HR information. Error: {str(e)}"
        # Printing error message to console
        print(error_message)
        return error_message
tools = [
    (get_hr_information_def, get_hr_information_handler)
]
